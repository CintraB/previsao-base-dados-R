...
#treinando com floresta aleatoria
any(is.na(treino_novo80)) #procurando NA em treino
treino_novo80 <- na.omit(treino_novo80)

any(is.na(teste_novo20)) #procurando NA em teste
teste_novo20 <- na.omit(teste_novo20)

rows.to.delete <- 0.2 * nrow(treino_novo80) #deixei só 20%
treino_novo80 <- treino_novo80[1:rows.to.delete, ] 

rows.to.delete <- 0.2 * nrow(teste_novo20)
teste_novo20 <- teste_novo20[1:rows.to.delete, ] #deixei só 20%

#coloco tudo em factor (os is_listened) pra rodar
treino_novo80$is_listened <- factor(treino_novo80$is_listened)
teste_novo20$is_listened <- factor(teste_novo20$is_listened)
teste$is_listened <- factor(teste$is_listened)

#treina
mod_rf <- randomForest(formula = is_listened~.,data = treino_novo80, ntree = 3)
#classifica
